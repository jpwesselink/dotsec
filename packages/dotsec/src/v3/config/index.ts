import { CreateConfig, DefaultConfig, DotsecConfig } from "../types";
			...
			return (userConfigs || []).reduce<DefaultConfig>(
				(firstConfig, secondConfig) => {
					return {
						...
						...secondConfig,
						defaults: {
							...firstConfig.defaults,
							...secondConfig?.defaults,
							aws: {
								...firstConfig.defaults.aws,
								...secondConfig?.defaults?.aws,
								kms: {
									...firstConfig.defaults.aws?.kms,
									...secondConfig?.defaults?.aws?.kms,
								},
								ssm: {
									...firstConfig.defaults.aws?.ssm,
									...secondConfig?.defaults?.aws?.ssm,
								},
								secretsManager: {
									...firstConfig.defaults.aws?.secretsManager,
									...secondConfig?.defaults?.aws?.secretsManager,
								},
							},
						},
						commands: {
