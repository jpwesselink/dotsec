...
import { FFValue, flatten } from '../tree';
import {
    AWSVariablesConfigRequired,
...
export const getAWSVariablesFromFile = async ({
    defaultConfig,
    options,
}: {
    defaultConfig: AWSVariablesConfigRequired;
    options?: { filename?: string };
}): Promise<AWSVariablesConfigByType> => {
    ...
    if (configPath) {
        ...
        if (configPath.endsWith('.json')) {
            configType = 'json';
            const data = (await loadJson(configPath)) as AWSVariables;
            const validatedConfig: AWSVariablesConfigRequired = {
                config: {
                    aws: {
                        regions:
            ...
            return {
                ...
                config: {
                    parameterStore: data?.parameterStore
                        ? flatten(data.parameterStore as FFValue)
                        : undefined,
                    ...validatedConfig,
                },
            };
...
export const getAWSEncryptedVariablesFromFile = async ({
    defaultConfig,
    options,
}: {
    defaultConfig: AWSVariablesConfigRequired;
    options?: { filename?: string };
}): Promise<AWSEncryptedVariablesConfigByFileType> => {
    const { filename } = options || {};
    ...
    if (configPath) {
        ...
        if (configPath.endsWith('.json')) {
            configType = 'json';
            const data = (await loadJson(configPath)) as AWSEncryptedVariables;
            const validatedConfig: AWSVariablesConfigRequired = {
                config: {
                    aws: {
                        regions:
